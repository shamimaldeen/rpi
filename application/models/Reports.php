...
//    =============== its for sales_report_category_wise_count =============
    ...
    public function retrieve_dateWise_salarySheet($gdate, $per_page, $page)
    {
           $this->db->select('count(DISTINCT(pment.emp_sal_pay_id)) as emp_sal_pay_id, pment.*,p.id as employee_id,p.first_name,p.last_name, p.hrate as gross_salary, d.designation')
            ->from('employee_salary_payment pment')
            ->join('employee_history p', 'pment.employee_id = p.id', 'left')
            ->join('designation d', 'p.designation = d.id', 'left')
            ->where('pment.salary_month', $gdate)
...
//        echo $this->db->last_query(); exit;
    ...
    public function count_dateWise_salarySheet($gdate)
    {
        $this->db->select('count(DISTINCT(pment.emp_sal_pay_id)) as emp_sal_pay_id, pment.*,p.id as employee_id,p.first_name,p.last_name')
        ->from('employee_salary_payment pment')
        ->join('employee_history p', 'pment.employee_id = p.id', 'left')
        ->where('pment.salary_month', $gdate)
        ->group_by('pment.emp_sal_pay_id')
