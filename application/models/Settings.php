...
//count office loan
    ...
    public function select_advance_salary_by_id($id) {
        $this->db->select('*');
    ...
    public function advance_salary_list_limit($per_page, $limit) {
    ...
    $this->db->limit($per_page, $limit);
    $this->db->order_by('salary_advance.id', 'DESC');
    $query = $this->db->get();
    if ($query->num_rows() > 0) {
        return $query->result_array();
    }
    ...
    public function search($id,$start_date,$end_date) {
        ...
        $this->db->from('salary_advance');
        $this->db->join('employee_history', 'employee_history.id = salary_advance.employee_id', 'left');
        $this->db->where('salary_advance.employee_id',$id);
        $this->db->where('salary_advance.taken_date >=', $start_date);
        $this->db->where('salary_advance.taken_date <=', $end_date);
        $this->db->order_by('salary_advance.id', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
    ...
    public function advance_salary($id) {
        $this->db->select('employee_history.*');
        $this->db->from('salary_advance');
        $this->db->join('employee_history', 'employee_history.id = salary_advance.employee_id', 'left');
        $this->db->where('salary_advance.employee_id',$id);
       // $this->db->order_by('salary_advance.id', 'DESC');
    ...
    public function employee_list() {
        $this->db->select('employee_history.first_name, employee_history.last_name,employee_history.id');
        $this->db->from('salary_advance');
        $this->db->join('employee_history', 'employee_history.id = salary_advance.employee_id', 'left');
        $this->db->order_by('employee_history.first_name', 'DESC');
        $query = $this->db->get();
        if ($query->num_rows() > 0) {
            return $query->result_array();
        }
    ...
    public function update_advance_salary($data, $id) {
        $this->db->where('id', $id);
        $result = $this->db->update('salary_advance', $data);
        if ($result) {
            return true;
        } else {
            return false;
...
// update loan Person
    ...
    public function submit_advance_salary($data) {
        $result = $this->db->insert('salary_advance', $data);
        if ($result) {
            return TRUE;
        } else {
            return false;
  ...
  public function delete_personal_loan($id = null)
    ...
    {
        $this->db->where('id',$id)->delete('salary_advance');
        if ($this->db->affected_rows()) {
            return true;
        } else {
            return false;
