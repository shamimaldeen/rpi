...
class Invoices extends CI_Model
{
    ...
    public function getInvoiceList($postData = null)
    {
        ...
        foreach ($records as $record) {
            ...
            $button .= '  <a href="' . $base_url . 'Cinvoice/invoice_callan_inserted_data/' . $record->invoice_id . '" class="btn btn-warning btn-sm" data-toggle="tooltip" data-placement="left" title="' . display('callan') . '"><i class="fa fa-desktop" aria-hidden="true"></i></a>';
            ...
            $button .= '  <a href="' . $base_url . 'Cinvoice/invoice_delete/' . $record->invoice_id . '" class="btn btn-danger btn-sm" class="" data-toggle="tooltip"
                           data-placement="right" title="'.display('delete').'" onclick="return confirm(\'Are you sure you want to delete this item?\');"><i class="fa fa-trash-o" aria-hidden="true"></i></a>';
...
// count invoice search by customer
    ...
    public function invoice_entry()
    {
        ...
        $datainv = array(
            ...
            'invoice_details' => (!empty($this->input->post('inva_details', TRUE)) ? $this->input->post('inva_details', TRUE) : 'THANK YOU OF YOUR BUSINESS'),
...
// Cash in Hand debit
        ...
        $dove_n = $this->input->post('dove', TRUE);
        $pkt_n = $this->input->post('pkt', TRUE);
...
//            $data2['serial']         = $html;
    ...
    //Retrieve invoice_html_data
    public function retrieve_datewise_invoice_data($from_date, $to_date, $customer_id)
    {
        $this->db->select('a.total_tax,
                        a.*,
                        b.*,
    ...
    public function retrieve_customer_previous_due($customer_id)
    {
        $this->db->select('prevous_due');
