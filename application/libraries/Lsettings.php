...
class Lsettings {
    ...
    public function add_advance() {
        ...
        $data = array(
            'title'       => display('add_advance'),
            'employee_list' => $employee_list,
        );
        $add_adv = $CI->parser->parse('settings/add_advance', $data, true);
        return $add_adv;
    }
    //Manage Advance Salary information
    public function manage_advance_salary($links, $per_page, $limit) {
        $CI = & get_instance();
        $CI->load->model('Settings');
        ...
        $data = array(
            ...
            'advance_list' => $as_list,
            'employee_list' => $list,
            'links'       => $links,
            'currency'    => $currency_details[0]['currency'],
            'position'    => $currency_details[0]['currency_position'],
        );
        $manage_as = $CI->parser->parse('settings/advance_salary_manage', $data, true);
        return $manage_as;
    }
 ...
 $currency_details = $CI->Web_settings->retrieve_setting_editdata();
    ...
    public function edit_advance_salary($id) {
        ...
        $CI->load->model('Settings');
        $CI->load->model('Hrm_model');
        $list = $CI->Settings->select_advance_salary_by_id($id);
        $employee_list = $CI->Hrm_model->employee_list_dropdown();
        $data = array(
            'title'          => display('edit_advance_salary'),
            'employee_list'  => $employee_list,
            'id'             => $list['id'],
            'employee_id'    => $list['employee_id'],
            'amount'         => $list['amount'],
            'taken_date'     => $list['taken_date'],
            'details'        => $list['details'],
        );
        $edit = $CI->parser->parse('settings/edit_advance', $data, true);
        return $edit;
    }
