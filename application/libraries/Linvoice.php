...
class Linvoice {
    ...
    public function invoice_callan_html_data($invoice_id) {
        $CI = & get_instance();
        $CI->load->model('Invoices');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
...
//        dd($total_qty);
        ...
        $company_info = $CI->Invoices->retrieve_company();
        $quantity_inword = $CI->numbertowords->convert_number($total_qty);
        $user_id = $invoice_detail[0]['sales_by'];
        $users = $CI->Invoices->user_invoice_data($user_id);
        $data = array(
        'title'             => display('invoice_details'),
        ...
        'invoice_all_data'  => $invoice_detail,
        'company_info'      => $company_info,
        'quantity_inword'   => $quantity_inword,
        'users_name'        => $users->first_name.' '.$users->last_name,

        );

        $chapterList = $CI->parser->parse('invoice/invoice_callan_html', $data, true);
        return $chapterList;
    }
    ...
    public function datewise_invoice_data($from_date, $to_date, $customer_id, $invoiceNo) {
        $CI = & get_instance();
        $CI->load->model('Invoices');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
        ...
        if ($from_date && $to_date && $customer_id) {
            $invoice_detail = $CI->Invoices->retrieve_datewise_invoice_data($from_date, $to_date, $customer_id);
//            dd($invoice_detail);
            $previous = $CI->Invoices->retrieve_customer_previous_due($customer_id);
            $taxfield = $CI->db->select('*')->from('tax_settings')->where('is_show', 1)->get()->result_array();
            foreach ($taxfield as $txrgname) {
                $regname = $txrgname['tax_name'] . ' Reg No  - ' . $txrgname['reg_no'] . ', ';
                $txregname .= $regname;
            }
...
//                    $previous = $invoice_detail[$k]['prevous_due'];
                    if ($temp_inv != $invoice_detail[$k]['invoice_id']) {
                        $temp_inv = $invoice_detail[$k]['invoice_id'];
                        $total_amount      += $invoice_detail[$k]['total_amount'];
                        $total_discount    += $invoice_detail[$k]['total_discount'];
                        $total_tax         += $invoice_detail[$k]['total_tax'];
                        $paid_amount       += $invoice_detail[$k]['paid_amount'];
                        $due_amount        += ($invoice_detail[$k]['total_amount']-$invoice_detail[$k]['paid_amount']);
                        $shipping_cost     += $invoice_detail[$k]['shipping_cost'];
                    }
                    if (isset($invoice_merge[$v['product_id']])){
                        $invoice_merge[$v['product_id']]['total_price'] += $v['total_price'];
                        $invoice_merge[$v['product_id']]['quantity'] += $v['quantity'];
                    }else{
                        $j++;
                        $invoice_merge[$v['product_id']] = $v;
                        $invoice_merge[$v['product_id']]['sl'] = $j;
                        if (!$product_id){
                            $invoice_merge[$v['product_id']]['final_date'] = $CI->occational->dateConvert($invoice_detail[$k]['date']);
...
//            dd($invoice_merge);
            $totalbal = $total_amount+$previous;
            $amount_inword = $CI->numbertowords->convert_number($subTotal_ammount);
            $user_id = $invoice_detail[0]['sales_by'];
            $users = $CI->Invoices->user_invoice_data($user_id);
            $users_name = $users->first_name.' '.$users->last_name;
        }
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        $company_info = $CI->Invoices->retrieve_company();
        $all_customer = $CI->db->select('*')->from('customer_information')->get()->result();
        $data = array(
            'title'             => display('datewise_sale_details'),
            'all_customer'      => $all_customer,
//            'invoice_id'        => $invoice_detail[0]['invoice_id'],
            'invoice_no'        => $invoiceNo,
            'invoice_all_data'  => $invoice_merge,
            ...
            'due_amount'        => number_format($due_amount+$previous, 2, '.', ','),
            'previous'          => number_format($previous, 2, '.', ','),
            'shipping_cost'     => number_format($shipping_cost, 2, '.', ','),
        );

        $chapterList = $CI->parser->parse('invoice/datewise_invoice', $data, true);
        return $chapterList;
    }
