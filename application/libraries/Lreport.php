...
// Tax report 
    ...
    // ======================= Salary Sheet Report ===============================
    public function retrieve_dateWise_salarySheet($gdate, $links, $per_page, $page) {
        $CI = & get_instance();
        $CI->load->model('Reports');
        $CI->load->model('Web_settings');
        $CI->load->library('occational');
        $CI->load->library('numbertowords');
        $salary_payment = $CI->Reports->retrieve_dateWise_salarySheet($gdate, $per_page, $page);
        $total_salary = 0;
        if (!empty($salary_payment)) {
            $i = 0;
            foreach ($salary_payment as $k => $v) {
                $i++;
                $salary_payment[$k]['sl'] = $i;
                $total_salary += $salary_payment[$k]['total_salary']-$v['advance_detuct'];
            }
        }
        $total_word = $CI->numbertowords->convert_number($total_salary);
        $currency_details = $CI->Web_settings->retrieve_setting_editdata();
        $company_info = $CI->Reports->retrieve_company();
        $data = array(
            'title'        => display('salary_sheet'),
            'total_salary' => $total_salary,
            'total_word'     => $total_word,
            'salary_sheet' => $salary_payment,
            'company_info' => $company_info,
            'myDate'        => $gdate,
            'currency'     => $currency_details[0]['currency'],
            'position'     => $currency_details[0]['currency_position'],
        ...
        $reportList = $CI->parser->parse('report/salary_sheet_report', $data, true);
        return $reportList;
    }
