...
// date wise product report
    ...
    // ===================== Salary Sheet Report=============================
    public function salary_sheet_dateWise() {
        $CI = & get_instance();
        $this->auth->check_admin_auth();
        $CI->load->library('lreport');
        $CI->load->model('Reports');
        $mydate = $this->input->get('myDate',TRUE);
        if ($mydate) {
            list($month,$year) = explode(' ',$mydate);
            $query = $this->db->select('*')->from('salary_sheet_generate')->where('gdate', $mydate)->get()->num_rows();
            if (!$query) {
                $this->session->set_userdata(array('error_message' => display('the_salary_of') . $month . display('is_not_generated')));
                redirect(base_url('Creport/salary_sheet_dateWise'));
            }
        }
        $total_rows = $this->Reports->count_dateWise_salarySheet($mydate);
        $perpagdata = 50;
        $config["base_url"] = base_url('Admin_dashboard/salary_sheet_dateWise/');
        ...
        $content = $CI->lreport->retrieve_dateWise_salarySheet($mydate, $links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }
