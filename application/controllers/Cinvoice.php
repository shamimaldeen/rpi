...
// invoice list pdf download
    ...
    public function invoice_callan_inserted_data($invoice_id) {
        $CI = & get_instance();
        $CI->auth->check_admin_auth();
        $CI->load->library('linvoice');
        $content = $CI->linvoice->invoice_callan_html_data($invoice_id);
        $this->template->full_admin_html_view($content);
    }
...
//    =========== its for 1 increment =============
  ...
  $createdate=date('Y-m-d H:i:s');
    ...
    $customer_coa = [
        ...
        public function dateWiseInvoice()
        {
            $CI = & get_instance();
            $CI->auth->check_admin_auth();
            $CI->load->library('linvoice');
            ...
            $invoiceNo = $this->input->get('invoice_no', TRUE);
            $content = $CI->linvoice->datewise_invoice_data($from_date, $to_date, $customer_id, $invoiceNo);
            $this->template->full_admin_html_view($content);
        }
