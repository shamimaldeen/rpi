...
class Csettings extends CI_Controller {
    ...
    public function add_advance() {
        $content = $this->lsettings->add_advance();
        $this->template->full_admin_html_view($content);
    }
    public function submit_advance() {
        $data = array(
            'employee_id'       => $this->input->post('employee_id',TRUE),
            'amount'          => $this->input->post('amount',TRUE),
            'taken_date'      => $this->input->post('date',TRUE),
            'details'         => $this->input->post('details',TRUE),
        );
        $result = $this->Settings->submit_advance_salary($data);
        if ($result) {
            $this->session->set_userdata(array('message' => display('successfully_added')));
            redirect(base_url('Csettings/add_advance'));
        } else {
    ...
    public function manage_advance_salary() {
        ...
        #pagination starts
        #
        $config["base_url"] = base_url('Csettings/manage_advance_salary/');
        $config["total_rows"] = $this->Settings->advance_salary_list();
        $config["per_page"] = 10;
        ...
        #pagination ends
        #
        $content = $this->lsettings->manage_advance_salary($links, $config["per_page"], $page);
        $this->template->full_admin_html_view($content);
    }

    //Employee wise attendance report
    public function dateWiseSalaryAdvance(){

        $data['title']      = display('Date wise salary report');
        ...
        $data['query']      = $this->Settings->search($id,$start_date,$end_date);

        $content            = $this->parser->parse('settings/date_wise_salaryadvance', $data, true);
        $this->template->full_admin_html_view($content);
    ...
    public function advance_salary_edit($id) {
        $content = $this->lsettings->edit_advance_salary($id);
        $this->template->full_admin_html_view($content);
    }

    public function update_advance_salary($id) {
        $data = array(
            'employee_id'   => $this->input->post('employee_id',TRUE),
            'taken_date'    => $this->input->post('date',TRUE),
            'amount'        => $this->input->post('amount',TRUE),
        ...
        $result = $this->Settings->update_advance_salary($data, $id);
        if ($result) {
            $this->session->set_userdata(array('message' => display('successfully_updated')));
            redirect(base_url('Csettings/manage_advance_salary'));
        } else {
            $this->session->set_userdata(array('error_message' => display('not_added')));
            redirect(base_url('Csettings/manage_advance_salary'));
        }
    ...
    { 
     ...
     public function delete_advance_salary($id = null)
    {
        if ($this->Settings->delete_advance_salary($id)) {
            #set success message
            $this->session->set_flashdata('message',display('delete_successfully'));
        } else {
            #set exception message
